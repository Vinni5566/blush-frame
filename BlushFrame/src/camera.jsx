...
function Camera() {
      ...
      case "single":
        return (
          <div className="p-4 bg-white rounded-xl border-4 border-pink-400 shadow-lg flex justify-center items-center hover:scale-110 transition-transform">
            <div className={`w-60 h-40 ${photos[0] ? '' : 'bg-gray-200'}`}>
                {photos[0] ? (
      ...
      case "strip":
        return (
          <div className="flex flex-col gap-3 p-4 bg-white rounded-xl border-4 border-pink-400 shadow-lg hover:scale-110 transition-transform">
            {photos.map((photo, i) => (
              <div key={i} className={`w-45 h-25 ${photo ? '' : 'bg-gray-200'}`}>
      ...
      case "grid":
        return (
          <div className="grid grid-cols-2 gap-3 p-4 bg-white rounded-xl border-4 border-pink-400 shadow-lg w-96 h-96 hover:scale-110 transition-transform">
            {photos.map((photo, i) => (
              <div key={i} className={`${photo ? '' : 'bg-gray-200'} h-40`}>
  ...
  return (
    <div className=" fixed inset-0 overflow-hidden flex flex-col items-center justify-center gap-6 p-4 min-h-screen bg-gradient-to-br from-purple-100 to-pink-100">
      ...
      <div className="absolute top-3/4 left-73 right-22 w-24 h-24 bg-purple-300 rounded-full opacity-40 animate-bounce"></div>
      <div className="absolute top-3/4 right-51 w-32 h-32 bg-pink-300 rounded-full opacity-60 animate-bounce"></div>
